...
export default function HomePage(): React.ReactElement {
  return (
    <div className="min-h-screen">
      {/* ─── Navbar ────────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-50 border-b border-border/50" style={{ background: 'hsl(0 0% 4% / 0.9)', backdropFilter: 'blur(12px)' }}>
        <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" id="nav-logo" className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-white">Zonvo</span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/register"
              id="nav-register"
              className="px-4 py-2 rounded-lg text-sm font-semibold text-white btn-glow"
              style={{ background: 'linear-gradient(135deg, hsl(262 83% 67%), hsl(217 91% 60%))' }}
            >
              Get started free
            </Link>
      ...
      <section className="relative py-24 lg:py-36 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] rounded-full blur-3xl opacity-15" style={{ background: 'hsl(262 83% 67%)' }} />
        </div>

        <div className="relative max-w-5xl mx-auto px-6 text-center">
          <div
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-8"
            style={{ background: 'hsl(262 83% 67% / 0.1)', border: '1px solid hsl(262 83% 67% / 0.3)', color: 'hsl(262 83% 80%)' }}
          >
            <Zap className="w-3 h-3" />
            Now in public beta — free forever
          </div>

          <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-white leading-tight text-balance">
            The future of{' '}
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/register"
              id="hero-cta-primary"
              className="px-8 py-3.5 rounded-xl text-base font-semibold text-white btn-glow flex items-center gap-2"
              style={{ background: 'linear-gradient(135deg, hsl(262 83% 67%), hsl(217 91% 60%))' }}
            >
              Start for free
            ...
            <Link
              ...
              className="px-8 py-3.5 rounded-xl text-base font-semibold text-foreground hover:bg-white/5 transition-colors border border-border"
            >
              See how it works
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((feature, i) => (
              <div
                key={i}
                id={`feature-${i}`}
                className="glass-card p-6 space-y-3 hover:border-primary/20 transition-colors"
              >
                <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => (
              <div
                key={plan.id}
                id={plan.id}
                className={`glass-card p-6 flex flex-col gap-6 ${plan.highlighted ? 'border-primary/50' : ''}`}
                style={plan.highlighted ? { borderColor: 'hsl(262 83% 67% / 0.5)', background: 'hsl(262 83% 67% / 0.05)' } : {}}
              >
                {plan.highlighted && (
                  <div
                    className="text-center py-1 rounded-full text-xs font-semibold text-white -mt-2"
                    style={{ background: 'linear-gradient(135deg, hsl(262 83% 67%), hsl(217 91% 60%))' }}
                  >
                    Most Popular
                  </div>
                ...
                <Link
                  ...
                  className={`py-2.5 rounded-lg text-sm font-semibold text-center transition-all ${
                    plan.highlighted
                      ? 'text-white btn-glow'
                      : 'text-foreground border border-border hover:bg-white/5'
                  }`}
                  style={
                    plan.highlighted
      ...
      <footer className="border-t border-border/50 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, hsl(262 83% 67%), hsl(217 91% 60%))' }}>
              <Zap className="w-3.5 h-3.5 text-white" fill="white" />
            </div>
            <span className="font-bold text-white text-sm">Zonvo</span>
          </div>
